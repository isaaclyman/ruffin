...
Router.route('/', {
	onBeforeAction: function() {
		...
		if(!Meteor.userId() && !Meteor.loggingIn()) {
			...
			if(Meteor.user()) {
				Session.setPersistent('zip', Meteor.user().profile.zip);
...
Router.route('/set/user/:_userid/token/:_password', {
	...
	onBeforeAction: function() {
		Meteor.loginWithPassword({ id: this.params._userid },
								 ...
								 function(error) {
			if(error) {
				app.fail('setting_user', [error, Router.current().route, this.params], false);
			}
		});
		this.next();
	},
	...
	action: function() {
		...
		app.fail('login_failed', [Router.current().route, this.params, Meteor.user()]), false);
		return true;
	}
});
