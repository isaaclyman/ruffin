Router.route('/region/:_region/board/:_hobby', {
	...
	waitOn: function() {
		...
		if(!this.params._region || this.params._region === 'NaN') {
			app.fail('region_undefined', [Router.current().route, this.params], false);
			return false;
		}
		if(!this.params._hobby || this.params._hobby === 'undefined') {
			app.fail('hobby_undefined', [Router.current().route, this.params], false);
			return false;
		}

		// Check that parameters are valid
		if(!app.checkRegion(this.params._region)) {
			app.fail('region_invalid', [Router.current().route, this.params], false);
			return false;
		}
		if(!app.checkBoardName(this.params._hobby)); {
			app.fail('hobby_invalid', [Router.current().route, this.params], false);
			return false;
		}
